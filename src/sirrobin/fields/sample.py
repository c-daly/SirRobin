...
from sirrobin.fields.contracts import BoundedFieldSample, FieldSample
...
def _device_axis_coordinates(
    ...
    *,
    ...
    """Return fixed-shape coordinates and validity without a host decision."""
    ...
    if periodic:
        ...
        lower = torch.floor(cell).to(torch.int64)
        upper = torch.remainder(lower + 1, count)
        invalid = torch.zeros_like(value, dtype=torch.bool)
    else:
...
def sample_reservoir_device(
    ...
    *,
    ...
    """Sample a boundary-validated reservoir without a device-to-host branch."""

    position = positions_m.to(torch.float64)
    x0, x1, tx, _ = _device_axis_coordinates(
        position[..., 0], geometry.dx_m, geometry.gx, periodic=True
    )
    y0, y1, ty, _ = _device_axis_coordinates(
        position[..., 1], geometry.dy_m, geometry.gy, periodic=True
    )
    z0, z1, tz, invalid_z = _device_axis_coordinates(
        -position[..., 2], geometry.dz_m, geometry.gz, periodic=False
    )
    world = torch.arange(reservoir_q.shape[0], device=reservoir_q.device)[:, None]

    def gather(ix: torch.Tensor, iy: torch.Tensor, iz: torch.Tensor) -> torch.Tensor:
        return (
            reservoir_q[world, ix, iy, iz].to(torch.float64)
            * q_mass_mol
    ...
    return BoundedFieldSample(
        value,
        torch.stack((dx, dy, -depth_gradient), dim=-1),
        invalid_z,
    )
